/* FAU200 Info Command - Version 2.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    if (err == FAU200Device::SUCCESS) {  // Device was successfully opened
        ...
        if (errcnt > 0) {  // In case of error
            ...
            std::cout << "Hardware revision: " << FAU200Device::hardwareRevision(config) << " [0x" << std::hex << std::setfill ('0') << std::setw(4) << (config.majrel << 8 | config.minrel) << std::dec << "]" << std::endl;  // Print hardware revision

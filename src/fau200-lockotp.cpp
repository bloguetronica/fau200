/* FAU200 LockOTP Command - Version 2.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    if (argc < 2) {  // If the program was called without arguments
        std::cerr << "Error: Missing argument.\nUsage: fau200-lockotp SERIALNUMBER\n";
        errlvl = EXIT_USERERR;
    } else {
        CP2130 cp2130;
        int err = cp2130.open(FAU200Device::VID, FAU200Device::PID, argv[1]);
        if (err == FAU200Device::SUCCESS) {  // Open the device having the specified serial number, and get the device handle. If successful
            int errcnt = 0;
            std::string errstr;
